...
class SHAPE:
    ...
    def load(cls, checkpoint: str, device: str = "cpu", precision: str = "fp32") -> "SHAPE":
        ...
        # The heads and the phys buffers are per corpus; the trunk is not. Only
        # the trunk is reused, so the checkpoint's own cards are rebuilt from the
        # shapes it carries rather than from the corpora on disk.
        model = Shape([cls._card_from_state(sd)], hidden_dim=meta["hidden"],
                      num_layers=meta["layers"], relations=meta.get("relations", "legacy"),
    ...
    def _card_from_state(sd: dict) -> DatasetCard:
        ...
        return DatasetCard(name="_init", task=task, metric="rocauc" if task == "graph-classify"
                           else "masked_mae", num_nodes=1, W=12, H=1, C=1,
    ...
    def _check_window(self, w: int) -> None:
        ...
        if p:
            if w % p:
                raise ValueError(f"window {w} is not a multiple of patch length {p}")
        elif w != self.meta.get("window", w):
...
def _self_check() -> None:
    """Round-trip both entry points on a checkpoint if one is present."""
    import glob
    ck = sorted(glob.glob("runs/ms/ms-p12-s0/seed0/best.ckpt"))
    if not ck:
        print("no checkpoint on disk; skipping"); return
    m = SHAPE.load(ck[0])
    y = m.forecast(torch.rand(40, 12) * 50 + 100, horizon=12)
    ...
    print(f"  forecast [40,12] -> {tuple(y.shape)}  mean {y.mean():.2f}  OK")
